...
std::string findPolyTail_single(const std::string& sequence, char poly_base, int window_size, int min_count) {
  int max_start = -1;
  int max_end = -1;
  int max_length = 0;
  poly_base = std::toupper(poly_base);
  for (int i = 0; i <= sequence.length() - window_size; ++i) {
    int count = 0;
    for (int j = i; j < i + window_size; ++j) {
      if (std::toupper(sequence[j]) == poly_base) {
        ++count;
      }
    }
    if (count >= min_count) {
      ...
      // Extend the current window to the right as long as the base matches poly_base
      while (current_end + 1 < sequence.length() && std::toupper(sequence[current_end + 1]) == poly_base) {
        ++current_end;
      }
      // Update the maximum poly tail found
      int current_length = current_end - current_start + 1;
      if (current_length > max_length) {
        max_start = current_start;
        max_end = current_end;
        max_length = current_length;
      }
      // Move the outer loop index to the end of the current poly tail to avoid redundant checks
      i = current_end;
    }
  }
  
  if (max_start != -1) {
    std::string poly_type = (poly_base == 'A') ? "poly_a" : "poly_t";
    return poly_type + ":0:" + std::to_string(max_start + 1) + ":" + std::to_string(max_end + 1);
  }
...
std::string findPolyTails(const std::string& sequence, char poly_base, int window_size, int min_count) {
  std::vector<std::string> poly_tails;
  poly_base = std::toupper(poly_base);
  ...
  for (int i = 0; i <= sequence.length() - window_size; ++i) {
    ...
    if (count >= min_count) {
      ...
      std::string poly_type = (poly_base == 'A') ? "poly_a" : "poly_t";
      poly_tails.push_back(poly_type + ":0:" + std::to_string(current_start + 1) + ":" + std::to_string(current_end + 1));
      // Move the outer loop index to the end of the current poly tail to avoid redundant checks
      i = current_end;
    }
  }
  // Concatenate all poly-tails into a single string
  std::ostringstream result;
  for (const auto& tail : poly_tails) {
    if (!result.str().empty()) {
      result << "|";
    }
    result << tail;
...
// [[Rcpp::export]]
Rcpp::DataFrame sigalign_stats(
    Rcpp::CharacterVector adapters,
    std::vector<std::string> sequences, int nthreads = 1) {
  std::vector<std::string> queries = Rcpp::as<std::vector<std::string>>(adapters);
...
#pragma omp parallel for
  for (int i = 0; i < sequences.size(); ++i) {
    ...
    for (int j = 0; j < filtered_queries.size(); ++j) {
      ...
      const auto& query_name = Rcpp::as<std::string>(filtered_query_names[j]);
